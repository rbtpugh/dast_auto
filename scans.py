...
class ase(object):  
   ...
   #Start a new job  
      ...
      while state != 'Running':  #If the application is not yet running wait for  waitTime second then check again.   
         ...
         #just in case the report started and completed before the this loop started, check the last-run tag and compare it to current time
         #this may take some tweaking depending on how the server time reports compared to the time ASE set on the report pack
         test=ET.fromstring(r.text[3:])
         dt_elem=root.findall(preTag+"last-run")
         datetimestr = dt_elem.pop().text
         dt = parse(datetimestr)
         dt_now = datetime.now(timezone.utc)
         dt_diff = dt_now - dt
         if dt_diff.seconds < 120:
             break
      print("Done")
   ...
   def uploadHTD(self, htdPath, folderItemId):
      ...
      for retries in range(5):
         try:
            r = self.session.post(url=self.server+'/services/folderitems/'+folderItemId+'/httptrafficdata?includeformfills=1', data=htdData, headers={'Content-Type': 'application/octet-stream'})     
         except (Exception) as e:
            self.connectionErrors += 1
            time.sleep(2)
            continue
